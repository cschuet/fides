...
BlobStore::BlobStore(const std::string& storage_path)
  ...
  DCHECK_EQ(absl::StrFormat(kFormatBlobFilename, 0).length(),
            kBlobFilenameLength);
}
...
std::string BlobStore::GetBlobPath(unsigned int blob_id,
  ...
  if (source_path.empty())
    return std::string();
  std::string filename = absl::StrFormat(kFormatBlobFilename, blob_id);
...
std::string BlobStore::GetSourcePath(const std::string& source_id) const {
  ...
  return absl::StrFormat(kFormatSourcePath, storage_path_.c_str(),
                            source_id.c_str());
}
